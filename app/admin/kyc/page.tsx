          ...
          {tabs.map((status) => (
            <button
              ...
              className={`px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-semibold transition-all whitespace-nowrap ${
                filter === status ? "bg-white/10 text-white" : "text-white/60 hover:text-white/70"
              }`}
            >
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="md:hidden space-y-3">
              {kycs.map((kyc) => (
                <div key={kyc.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
              ...
              <section>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
                  {modalDocs.map((doc) => (
                    <a
                      key={doc.label}
                      href={blobUrls[doc.label] || "#"}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="group block"
                      onClick={(e) => {
                        if (!blobUrls[doc.label]) e.preventDefault();
                      }}
                    >
