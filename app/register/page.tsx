...
function Field({ label, hint, children, index = 0 }: FieldProps) {
  return (
    ...
    >
      {label && (
        <label className="block text-[11px] font-bold hover:border-white/35 uppercase tracking-[0.12em] mb-2">
          {label}
      ...
      <div className="flex-1 flex items-center justify-center px-5 py-12 relative">
        ...
        <div className="relative z-10 w-full max-w-110">
          ...
          <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="mb-8">
            ...
            <p className="hover:border-white/35 text-sm">Start your investment journey today</p>
          </motion.div>
                ...
                >
                  ...
                  <div className="flex gap-3 mt-2">
                    <button type="button" onClick={() => setStep(1)}
                      className="px-5 py-4 rounded-2xl text-sm font-semibold text-white/60 hover:text-white/70 border border-white/8 hover:border-white/18 transition-all">
                      Back
                    </button>
